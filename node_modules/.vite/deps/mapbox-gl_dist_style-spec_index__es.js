...
// node_modules/mapbox-gl/dist/style-spec/index.es.js
var $version = 8;
var $root = {
  version: {
    required: true,
...
function format(style, space = 2) {
  ...
  if (style.layers) {
    style.layers = style.layers.map((layer2) => sortKeysBy(layer2, v8.layer));
  }
...
function eachLayer(style, callback) {
  for (const layer2 of style.layers) {
    callback(layer2);
  }
}
function eachProperty(style, options, callback) {
  function inner(layer2, propertyType) {
    const properties = layer2[propertyType];
    if (!properties)
      return;
...
function eachLayout(layer2, callback) {
...
function eachPaint(layer2, callback) {
  for (const k in layer2) {
...
function migrateToV8(style) {
  style.version = 8;
  eachSource(style, (source2) => {
    if (source2.type === "video" && source2.url !== void 0) {
      renameProperty(source2, "url", "urls");
    }
    if (source2.type === "video") {
      source2.coordinates.forEach((coord) => {
        return coord.reverse();
      });
    }
  });
  eachLayer(style, (layer2) => {
    eachLayout(layer2, (layout2) => {
      if (layout2["symbol-min-distance"] !== void 0) {
        renameProperty(layout2, "symbol-min-distance", "symbol-spacing");
      }
    });
    eachPaint(layer2, (paint2) => {
      if (paint2["background-image"] !== void 0) {
        renameProperty(paint2, "background-image", "background-pattern");
      }
      if (paint2["line-image"] !== void 0) {
        renameProperty(paint2, "line-image", "line-pattern");
      }
      if (paint2["fill-image"] !== void 0) {
        renameProperty(paint2, "fill-image", "fill-pattern");
      }
    });
  });
  eachProperty(style, {
  ...
  eachLayer(style, (layer2) => {
    eachLayout(layer2, (layout2) => {
      delete layout2["text-max-size"];
      delete layout2["icon-max-size"];
    });
    eachPaint(layer2, (paint2) => {
      if (paint2["text-size"]) {
        if (!layer2.layout)
          layer2.layout = {};
        layer2.layout["text-size"] = paint2["text-size"];
        delete paint2["text-size"];
      }
      if (paint2["icon-size"]) {
        if (!layer2.layout)
          layer2.layout = {};
        layer2.layout["icon-size"] = paint2["icon-size"];
        delete paint2["icon-size"];
      }
    });
  });
  ...
  eachLayer(style, (layer2) => {
    eachLayout(layer2, (layout2) => {
      if (layout2["text-font"]) {
        layout2["text-font"] = migrateFontStack(layout2["text-font"]);
      }
    });
  });
...
var ParsingError$1 = class extends Error {
...
var Color = class {
  ...
  static parse(input) {
    ...
    const rgba2 = parseCSSColor_1(input);
    if (!rgba2) {
      return void 0;
    }
...
var Coercion = class {
  ...
  evaluate(ctx) {
    if (this.type.kind === "boolean") {
      ...
      throw new RuntimeError$1(error2 || `Could not parse color from value '${typeof input === "string" ? input : String(JSON.stringify(input))}'`);
...
var CompoundExpression = class {
  ...
  static register(registry, definitions2) {
    CompoundExpression.definitions = definitions2;
    for (const name in definitions2) {
...
var ParsingContext = class {
  ...
  error(error2, ...keys) {
    ...
    this.errors.push(new ParsingError$2(key, error2));
  }
  /**
...
function isConstant(expression2) {
  if (expression2 instanceof Var$1) {
    return isConstant(expression2.boundExpression);
  } else if (expression2 instanceof CompoundExpression$1 && expression2.name === "error") {
    return false;
  } else if (expression2 instanceof CollatorExpression) {
    return false;
  } else if (expression2 instanceof Within$1) {
    return false;
  }
  const isTypeAnnotation = expression2 instanceof Coercion$1 || expression2 instanceof Assertion$1;
...
var NumberFormat = class {
  ...
  // Default 0
  // Default 3
  constructor(number2, locale, currency, unit, minFractionDigits, maxFractionDigits) {
...
function rgba(ctx, [r, g, b, a]) {
  ...
  if (error2)
    throw new RuntimeError$1(error2);
...
function createFunction(parameters, propertySpec) {
  ...
  if (zoomAndFeatureDependent) {
    ...
    return {
      kind: "composite",
      interpolationType,
      interpolationFactor: Interpolate$1.interpolationFactor.bind(void 0, interpolationType),
...
function isExpression(expression2) {
  return Array.isArray(expression2) && expression2.length > 0 && typeof expression2[0] === "string" && expression2[0] in definitions;
}
function createExpression(expression2, propertySpec) {
  const parser = new ParsingContext$1(definitions, [], propertySpec ? getExpectedType(propertySpec) : void 0);
...
function createPropertyExpression(expression2, propertySpec) {
  ...
  const interpolationType = zoomCurve instanceof Interpolate$1 ? zoomCurve.interpolation : void 0;
...
function findZoomCurve(expression2) {
  let result = null;
  if (expression2 instanceof Let$1) {
    result = findZoomCurve(expression2.result);
  } else if (expression2 instanceof Coalesce$1) {
  ...
  } else if ((expression2 instanceof Step$1 || expression2 instanceof Interpolate$1) && expression2.input instanceof CompoundExpression$1 && expression2.input.name === "zoom") {
    result = expression2;
  }
...
function getExpectedType(spec) {
  ...
  if (spec.type === "array") {
    return array$1(types2[spec.value] || ValueType, spec.length);
  }
...
function createFilter(filter2, layerType = "fill") {
...
function extractStaticFilter(filter2) {
  if (!isDynamicFilter(filter2)) {
    return filter2;
  }
  let result = deepUnbundle(filter2);
  unionDynamicBranches(result);
  result = collapseDynamicBooleanExpressions(result);
  return result;
}
function collapseDynamicBooleanExpressions(expression2) {
  if (!Array.isArray(expression2)) {
    return expression2;
  }
  const collapsed = collapsedExpression(expression2);
  if (collapsed === true) {
    return collapsed;
  } else {
...
function unionDynamicBranches(filter2) {
  let isBranchingDynamically = false;
  const branches = [];
  if (filter2[0] === "case") {
    for (let i = 1; i < filter2.length - 1; i += 2) {
      isBranchingDynamically = isBranchingDynamically || isDynamicFilter(filter2[i]);
      branches.push(filter2[i + 1]);
    }
    branches.push(filter2[filter2.length - 1]);
  } else if (filter2[0] === "match") {
    isBranchingDynamically = isBranchingDynamically || isDynamicFilter(filter2[1]);
    for (let i = 2; i < filter2.length - 1; i += 2) {
      branches.push(filter2[i + 1]);
    }
    branches.push(filter2[filter2.length - 1]);
  } else if (filter2[0] === "step") {
    isBranchingDynamically = isBranchingDynamically || isDynamicFilter(filter2[1]);
    for (let i = 1; i < filter2.length - 1; i += 2) {
      branches.push(filter2[i + 1]);
    }
  ...
  if (isBranchingDynamically) {
    filter2.length = 0;
    filter2.push("any", ...branches);
  }
  for (let i = 1; i < filter2.length; i++) {
    unionDynamicBranches(filter2[i]);
  }
}
function isDynamicFilter(filter2) {
  if (!Array.isArray(filter2)) {
    return false;
  }
  if (isRootExpressionDynamic(filter2[0])) {
    return true;
  }
  for (let i = 1; i < filter2.length; i++) {
    const child = filter2[i];
...
function isRootExpressionDynamic(expression2) {
  return expression2 === "pitch" || expression2 === "distance-from-center";
}
...
function collapsedExpression(expression2) {
  if (dynamicConditionExpressions.has(expression2[0])) {
    for (let i = 1; i < expression2.length; i++) {
      const param = expression2[i];
...
function convertFilter(filter2) {
  return _convertFilter(filter2, {});
}
function _convertFilter(filter2, expectedTypes) {
  if (isExpressionFilter(filter2)) {
    return filter2;
  }
  if (!filter2)
  ...
  if (op === "==" || op === "!=" || op === "<" || op === ">" || op === "<=" || op === ">=") {
    const [, property, value] = filter2;
    converted = convertComparisonOp(property, value, op, expectedTypes);
  } else if (op === "any") {
    const children = filter2.slice(1).map((f) => {
      const types2 = {};
      const child = _convertFilter(f, types2);
      const typechecks = runtimeTypeChecks(types2);
    ...
    const children = filter2.slice(1).map((f) => _convertFilter(f, expectedTypes));
    return children.length > 1 ? ["all"].concat(children) : [].concat(...children);
  } else if (op === "none") {
    return [
      "!",
      _convertFilter(["any"].concat(filter2.slice(1)), {})
    ];
  } else if (op === "in") {
    converted = convertInOp(filter2[1], filter2.slice(2));
  } else if (op === "!in") {
    converted = convertInOp(filter2[1], filter2.slice(2), true);
  } else if (op === "has") {
    converted = convertHasOp(filter2[1]);
  } else if (op === "!has") {
    converted = [
      "!",
      convertHasOp(filter2[1])
    ];
  } else {
    converted = true;
  }
...
function migrateToExpressions(style) {
  ...
  eachLayer(style, (layer2) => {
    if (layer2.filter) {
      layer2.filter = convertFilter(layer2.filter);
    }
  });
  eachProperty(style, {
    paint: true,
...
function composite(style) {
  ...
  for (const id in style.sources) {
    const source2 = style.sources[id];
    if (source2.type !== "vector")
      continue;
    const match = /^mapbox:\/\/(.*)/.exec(source2.url);
    if (!match)
      continue;
    styleIDs.push(id);
  ...
  style.layers.forEach((layer2) => {
    if (styleIDs.indexOf(layer2.source) >= 0) {
      layer2.source = compositeID;
      if ("source-layer" in layer2) {
        if (compositedSourceLayers.indexOf(layer2["source-layer"]) >= 0) {
          throw new Error("Conflicting source layer names");
        } else {
          compositedSourceLayers.push(layer2["source-layer"]);
        }
      }
    }
  });
...
function pluckId(layer2) {
  return layer2.id;
}
function indexById(group, layer2) {
  group[layer2.id] = layer2;
  return group;
}
...
function diffStyles(before, after) {
  ...
  try {
    ...
    const beforeLayers = [];
    if (before.layers) {
      before.layers.forEach((layer2) => {
        if (layer2.source && sourcesRemoved[layer2.source]) {
          commands.push({
            command: operations.removeLayer,
            args: [layer2.id]
          });
        } else {
          beforeLayers.push(layer2);
        }
      });
    }
...
function validateLayer(options) {
  ...
  if ("ref" in layer2) {
    ...
    if (!layer2.source) {
      ...
      } else if (type === "line" && layer2.paint && (layer2.paint["line-gradient"] || layer2.paint["line-trim-offset"]) && (sourceType !== "geojson" || !source2.lineMetrics)) {
    ...
    objectElementValidators: {
      ...
      paint(options2) {
        return validateObject({
          ...
          objectElementValidators: {
            "*"(options3) {
              return validatePaintProperty(extend({ layerType: type }, options3));
            }
          }
        });
      }
...
function getSourceTypeValues(styleSpec) {
  return styleSpec.source.reduce((memo, source2) => {
    const sourceType = styleSpec[source2];
...
function validateTerrain(options) {
  ...
  if (!terrain2.source) {
    ...
    if (!source2) {
      ...
      errors.push(new ValidationError(key, terrain2.source, `terrain cannot be used with a source of type ${String(sourceType)}, it only be used with a "raster-dem" source type`));
    }
  }
  return errors;
}
...
function validateProjection(options) {
  ...
  if (rootType === "object") {
    ...
    errors = errors.concat([new ValidationError("projection", projection2, `object or string expected, ${rootType} found`)]);
  }
  return errors;
}
var VALIDATORS = {
...
function getSourceCount(source2) {
  if (source2.url) {
    return source2.url.split(",").length;
  } else {
    return 0;
  }
}
...
var acceptedSourceTypes = /* @__PURE__ */ new Set([
  "vector",
  "raster",
  "raster-dem"
]);
function getSourceErrors(source2, i) {
  const errors = [];
  const sourceKeys = [
    "type",
    "url",
  ...
  errors.push(...getAllowedKeyErrors(source2, sourceKeys, "source"));
  if (!acceptedSourceTypes.has(String(source2.type))) {
    errors.push(new ValidationError(`sources[${i}].type`, source2.type, `Expected one of [${Array.from(acceptedSourceTypes).join(", ")}]`));
  }
  const sourceUrlPattern = /^mapbox:\/\/([^/]*)$/;
  if (!source2.url || !isValid(source2.url, sourceUrlPattern)) {
    errors.push(new ValidationError(`sources[${i}].url`, source2.url, "Expected a valid Mapbox tileset url"));
  }
  return errors;
}
function getSourcesErrors(sources2) {
  const errors = [];
  let count = 0;
  Object.keys(sources2).forEach((s, i) => {
    const sourceErrors = getSourceErrors(sources2[s], i);
    if (!sourceErrors.length) {
      count = count + getSourceCount(sources2[s]);
    }
...
*/
//# sourceMappingURL=mapbox-gl_dist_style-spec_index__es.js.map
